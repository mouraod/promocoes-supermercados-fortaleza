...
Roda `cometa-encartes`, `saoluiz-encartes` e `superdopovo-encartes` em sequência, sem precisar disparar uma por uma. Não substitui as skills individuais — elas continuam funcionando isoladas, e é o que usar quando o pedido for de uma rede só.

Reaproveita automaticamente (via hardlink) páginas de encartes que já foram baixados em rodadas anteriores, então rodadas repetidas na mesma semana são rápidas.
...
2. Roda `cometa-encartes` → `saoluiz-encartes` → `superdopovo-encartes`, um de cada vez (evita dois navegadores headless simultâneos).
...
# Resolução de rasterização (Cometa e Super do Povo; São Luiz não rasteriza)
...
Não existe `--output` aqui — ele apontaria as três redes pra mesma pasta, sobrescrevendo uma a outra. Pra isso, rode a skill da rede específica.

## Comportamento ao acionar

1. Executar o script direto — sem perguntas desnecessárias
2. Deixar a saída de cada rede rolar no terminal (é o `stdio: inherit` dos três scripts filhos)
...
5. Se uma rede falhar em tempo de execução (rede fora do ar, etc.), as outras duas continuam; reportar a falha exatamente como aparece no resumo
...
Quando o usuário pedir as melhores promoções das três redes juntas (ex: "quais as melhores promoções da semana?"), ler as imagens JPG das três pastas baixadas e apresentar os destaques.
...
Organizar por categoria com cabeçalho em negrito. Dentro de cada categoria, ordenar do maior pro menor desconto e indicar a rede de cada item (já que agora há três fontes). Incluir só itens com desconto relevante (acima de 15% ou preço muito bom). Exemplo:
